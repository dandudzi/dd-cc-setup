...
if not health_checks:
    st.info(
        ...
        "Go to **Turn Cost Asymmetry**, run a comparison, and click **Save as Health Check**."
    )
    st.stop()
...
with st.expander(label, expanded=True):
    ...
    for hc in health_checks:
        # Compute stats for this check's two categories
        if api_calls:
            result = compute_turn_cost(api_calls, [hc.category_a, hc.category_b])
            stats = result["stats"]
            sa = next(s for s in stats if s.category == hc.category_a)
            sb = next(s for s in stats if s.category == hc.category_b)
            actual = compute_actual_value(
                sa.mean_input_tokens if sa.n > 0 else None,
                sa.mean_content_length,
                sb.mean_input_tokens if sb.n > 0 else None,
                sb.mean_content_length,
                hc.metric,
            )
            status = compute_status(hc, actual)
            actual_str = f"{actual:+.3f}" if actual is not None else "—"
            samples = f"{sa.n + sb.n:,} turns"
        else:
            status = "INSUFFICIENT"
            actual_str = "—"
            samples = "no data"
        ...
        with st.container(border=True):
            ...
            if st.session_state.get(_edit_key, False):
                ...
                c1.metric("Category A", hc.category_a)
                c2.metric("Category B", hc.category_b)
                c3.metric("Metric", hc.metric.replace("_", " "))
                c4.metric("Actual", actual_str)
                c5.metric("Expected", f"{hc.expected:+.3f}" if hc.metric == "input_delta" else f"{hc.expected:.3f}")
